...
| RF-03 | Clasificar cada alerta según las categorías del caso de uso acotado | XDR §1 · LLM §1 | ~ El baseline determinista produce **4 de 6** categorías (incl. `fp_actividad_legitima` vía orígenes legítimos, RF-03); faltan `vp_acceso_consumado` y `vp_exposicion_gestion`, cuyas etiquetas finas el dataset no aporta (el clasificador se apoya en un **árbol de decisión** que valida y descubre reglas, no en un codificador con ajuste fino: es un límite de datos, no una pieza pendiente). La familia gobierna además el **nivel de respuesta** vía un registro extensible MITRE (`prototipo/familias.yml`): `actuar` (contener), `triar_y_enrutar` (clase `amenaza_enrutada`: se encamina a un equipo, sin contener — para amenazas que no deben auto-contenerse) o `no_soportada`. Ver [caso de uso §6](../01-fase1-analisis-del-modulo/caso-de-uso-acotado.md#los-tres-niveles-de-respuesta-y-el-registro-de-familias) |
...
| RF-05 | Generar por cada decisión una justificación estructurada: evidencia citada, hipótesis, técnica MITRE ATT&CK y acción sugerida | XDR §9 · LLM §1 | ✔ La traza emite `justificacion_estructurada` con los 4 componentes (`traza.py`), además del texto libre y la justificación con LLM+RAG |
| RF-06 | Emitir un nivel de confianza explícito junto a cada clasificación | XDR §9, §7 · LLM §4 | ✔ Es el disparador del escalado |
| RF-07 | Aplicar reglas y umbrales configurables que determinen cuándo se escala a validación humana | Fase 4 | ~ Reglas configurables en `perfil.continuidad`/`excepciones`; el **umbral de confianza** también (`continuidad.umbral_confianza`, 0.7 por defecto). Escalado medido en la campaña vigente (21/09): **0,297** (89 de 300): 20 por confianza (las ráfagas desde el origen de administración que la 6ª regla manda al humano; era 0,0 antes de esa regla) y 69 porque el bloqueo recae sobre un activo interno (`continuidad.actores`, D16). Su valor **calibrado** (barrido del umbral) sigue pendiente |
...
| RF-09 | Registrar traza auditable de cada ejecución: entrada, prompt, salida, decisión, veredicto del analista, marcas de tiempo y versión de prompt y modelo | XDR §9 · Fase 5 | ✔ La traza registra clase/acción, `justificacion(_estructurada)`, `veredicto_humano`/`clase_reclasificada`, y **`version_justificador`** (versión+modelo) + `pasajes_usados`; el prompt es reconstruible de (alerta, versión, pasajes). Base de la evaluación |
...
| RF-12 | Almacenar el feedback del analista en formato reutilizable, sin reentrenamiento | Trabajo futuro | ✔ Cubierto por RF-08: `veredicto_humano` y `clase_reclasificada` se persisten en la traza (JSONL), reutilizable sin reentrenar; su consumo futuro (los cuatro bucles: config, corpus RAG, reglas/árbol, evaluación) en [bucle de feedback](../00-general/bucle-de-feedback-rf12.md) |
| RF-13 | Exponer los resultados al sistema del cliente mediante una interfaz definida, sin sustituir sus funciones | XDR §5 | ~ La **traza JSONL es la interfaz de salida definida** (esquema estable de `traza.construir`), escrita **en tiempo real línea a línea** por el listener `prototipo/stream.py` (daemon MDR que sigue el `alerts.json` de Wazuh sin cerrarse, ver [COMO-PROBAR §3.bis](../../COMO-PROBAR.md)); el push al sistema del cliente es trabajo de producción (ejecutores por plano de control, ver [protocolos §7](../04-fase4-diseno-de-arquitectura/protocolos-comunicacion-sandbox.md)) |
...
| RF-17 | **Determinar y declarar el impacto sobre el servicio** de cada acción antes de proponerla: ninguno, localizado o alcanza a un servicio del cliente | Fase 1 · [C1](../01-fase1-analisis-del-modulo/modelo-de-cliente-generico.md#52-operativas-del-cliente--lo-que-el-prototipo-debe-respetar) | **+** Es la entrada de la política de continuidad y de RF-07. *Determinado (21/09):* `impacto.determinar` lo calcula desde el inventario del perfil —a quién bloquea la acción (`actores.quien_es`) y qué servicios detiene— y nunca por debajo del nivel del catálogo; queda en la traza (`impacto_determinado`) y en el prompt del analista |
...
| RF-19 | **Rechazar toda acción que dejaría el activo fuera del alcance del plano de gestión** | Fase 1 · [C6](../01-fase1-analisis-del-modulo/modelo-de-cliente-generico.md#52-operativas-del-cliente--lo-que-el-prototipo-debe-respetar) | **+** Precondición, no criterio de escalado: impediría la siguiente respuesta y la verificación. *Reparado (21/09):* la `ip_gestion` del perfil empresarial apuntaba a una IP inexistente; ahora es el auditor (`172.20.20.4`), el canal real del conector, y bloquearla es veto duro en `perfil.filtrar` |
| RF-20 | Calcular las **métricas de continuidad**: acciones disruptivas indebidas y retención correcta | Fase 1 · caso de uso §10 | **+** Sin ellas el informe no puede demostrar que se respetó la continuidad. *Ampliado (21/09):* `bloqueos_automaticos_indebidos` cuenta además los bloqueos sin humano sobre FP/PROPIA de cualquier impacto, el punto ciego del indicador |
...
| RNF-03 | **Reproducibilidad:** misma alerta y misma versión de prompt y modelo → misma decisión | XDR §9 · LLM §4 | ✔ Temp 0 (determinista) + **`version_justificador` (versión+modelo) registrado en la traza**; el prompt-builder es determinista dada la versión. Base comparable de la Fase 6 |
| RNF-04 | **Latencia:** tiempo de triaje del orden de segundos en el camino interactivo | XDR §2 | ~ Clasificar+decidir es sub-ms; justificar con el 1B por subprocess medía ~14–36 s (recarga del modelo por llamada). Con el **servidor residente** añadido después (`llama-server`, modelo ya en memoria) se elimina esa recarga. Documentado; no aplica al camino en lote |
...
| RNF-10 | **Mantenibilidad:** prompts, reglas y umbrales como configuración externa versionada | XDR §8 | ~ Reglas y umbrales sí son config externa versionada (perfiles YAML); los **prompts están en código** (`construir_prompt`) — externalizarlos es trabajo futuro |
| RNF-11 | **Usabilidad:** salida legible por un analista en menos de un minuto por alerta | XDR §1, §9 | ✔ |
| RNF-12 | **Cumplimiento:** política de retención y anonimización de los datos conforme a la normativa aplicable | XDR §8 | ~ Política documentada ([protocolos §8](../04-fase4-diseno-de-arquitectura/protocolos-comunicacion-sandbox.md)): las trazas llevan PII (IPs, usuarios en `evento_crudo`), retenidas localmente sin envío externo (RNF-01); anonimización/retención por normativa del cliente = config de despliegue, no implementada en el prototipo |
