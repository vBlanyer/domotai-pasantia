...
| RF-07 | Aplicar reglas y umbrales configurables que determinen cuándo se escala a validación humana | Fase 4 | ~ Reglas configurables en `perfil.continuidad`/`excepciones`; el **umbral de confianza** también (`continuidad.umbral_confianza`, 0.7 por defecto). Escalado medido en la campaña vigente: **0,067** (20 de 300, todas VP: las ráfagas desde el origen de administración que la 6ª regla manda al humano antes de bloquear al admin; era 0,0 antes de esa regla). Su valor **calibrado** (barrido del umbral) sigue pendiente |
