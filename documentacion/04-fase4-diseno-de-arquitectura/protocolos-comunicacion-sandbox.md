...
En una cadena FTTx no existe "el protocolo": cada tramo tiene el suyo, con dueño y superficie distintos.

| Tramo | Protocolo habitual | Naturaleza | ¿Aplicable aquí? |
|-------|--------------------|------------|------------------|
| OLT ↔ ONT | **OMCI** (ITU-T G.988) | Gestión interna del plano PON | No — fuera del alcance |
| Proveedor → CPE (heredado) | **TR-069 / CWMP** (SOAP sobre HTTP, puerto 7547) | Gestión remota masiva vía ACS | Solo con un ACS de por medio |
| Proveedor → CPE (moderno) | **TR-369 / USP** sobre MQTT, WebSocket o STOMP | Sucesor de TR-069 | Solo con infraestructura USP |
...
| TR-069 / CWMP | Solo CPE | Alta (en el ACS) | Sí, diferido | **Alto** | **ACS** |
| TR-369 / USP | Solo CPE | Alta | Sí | **Alto** | Controlador USP + broker |
...
TR-069 es el protocolo con el que un proveedor real gestiona su parque de CPE, y TR-369/USP es su sucesor. Diseñar sobre ellos sería el camino de máxima fidelidad operativa.
...
- Solo cubren el CPE. Los endpoints y el resto de nodos de la topología necesitarían un segundo canal de todas formas.
- **No hay hoy un proveedor concreto ni un parque de CPE definido**, así que no existe un ACS real al que alinearse. Diseñar contra un ACS hipotético corre el riesgo de acertar en la forma y fallar en el detalle.
...
Relevante además desde el punto de vista de seguridad: TR-069 tiene un historial documentado de abuso —el puerto 7547 expuesto ha sido vector de compromiso masivo de CPE— lo que refuerza que sea un **objeto de auditoría** en el sandbox, además de un canal de gestión potencial.
