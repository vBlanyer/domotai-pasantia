...
| 3 — Entorno de pruebas | **Completa** | Red del cliente de 7 nodos, Metasploitable con ground truth, Wazuh generando alertas reales, auditor Nmap normalizado, todo reproducible con `lab/lab.sh` y con [guía de instalación](../../lab/docs/instalacion.md). **Dataset de alertas etiquetado entregado** (`lab/dataset/etiquetado.jsonl`: 600 alertas etiquetadas —tres familias soportadas: acceso a credenciales, reconocimiento y servicio expuesto, más ruido de plataforma—, 174 VP / 38 FP / 8 PROPIA / 380 no_soportada, particionado). Laboratorio del banco (22/09): la red de `bancario.yml` con servicios y dependencias reales y monitor de salud; ver `lab/banco/`. Banco de pruebas del catálogo (§4): `python3 -m lab.banco.pruebas` — casos de clasificación, filtro y cascada sin lab, más los de extremo a extremo con `--con-vivo` (última corrida: 21 OK). Falta solo: equipo de borde OpenWrt real (vrnetlab bloqueado). |
...
| D11 | La interacción humana del prototipo es **por terminal (TUI/CLI)** y también por una **UI web local**: backend Python (`prototipo/tablero.py` con CORS, embebido por `stream.py --web`) y frontend **SPA React** (Vite/TS/shadcn/Tailwind/Zod) en `visor/`, pieza modular con la stack de la empresa que consume la API; observabilidad (salud, decisiones, trazas) y aprobación desde el navegador, ligada solo a `127.0.0.1` y **sin autenticación**. Login/TLS y la **concurrencia** (aprobación de doble canal web+terminal y contador en vivo, que exigen un hilo lector en el daemon de un solo hilo) quedan como **trabajo futuro** | [flujo §7](../04-fase4-diseno-de-arquitectura/flujo-triaje-playbook-sandbox.md) · [pruebas 09](../../docs/pruebas/09-tablero-web.md) |
