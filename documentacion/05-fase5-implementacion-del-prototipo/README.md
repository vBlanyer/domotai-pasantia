...
|-------|----------|------------------------|--------|
| **Ingesta y normalización** | Lee las alertas de la fuente y las lleva al esquema común, conservando la severidad de origen como baseline; **agnóstica de fuente** (adaptador inyectable, RNF-06) y tolerante a campos ausentes (RNF-07) | Una alerta real de `alerts.json` sale normalizada, con su activo resuelto aunque el equipo no tenga agente ([RF-16](../02-fase2-estado-del-arte/requisitos.md)); CLI verificada en vivo | **Hecho** — `prototipo/ingesta.py` (+ `adaptador_wazuh`) |
