...
#
# Uso:  sh lab/scripts/reenvio-syslog.sh [IP_DEL_MANAGER]   (por defecto la descubre)
...
docker exec -d clab-red-cliente-objetivo-vuln /usr/local/bin/reenvio.sh
sleep 2

if docker exec clab-red-cliente-objetivo-vuln sh -c 'ps aux | grep -q "[r]eenvio.sh"' 2>/dev/null; then
  echo "  reenvio activo"
  echo "Prueba:  docker exec clab-red-cliente-objetivo-vuln sh -c 'logger -p auth.info -t \"sshd[9]\" \"Failed password for root from 1.2.3.4 port 22 ssh2\"'"
else
